...
def reap_it(isbn)
	remove = ["alternate_edition", "link", "comments", "shipping_options"]

	response = Valorebooks::ProductAPI.new.get_products(ProductCode: isbn, Level: "detailed", NumberToShow: 5)
	product = response.xml["product"]

	unless product["sale_offer"].is_a? Array
	 	product["sale_offer"] = [product["sale_offer"]]
	end 

	out = []
	product["sale_offer"].each do |offer|
		out << offer.reject{ |k,v| remove.include? k  }
	end
	out
end
